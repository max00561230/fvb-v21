...
const QA_DIR = path.join(ROOT, "qa/v22.1-photo-tool-repair");
...
async function runDesktop(inputs) {
  ...
  const helpLabels = ["Quick Start", "Select a Page", "Upload a Photo", "Move and Resize", "Rotate and Crop", "Preview", "Save and Reopen a Draft", "Approve a Restoration", "Export the Restoration Package", "Lock Admin", "Where Drafts Are Stored", "What Happens After Export", "Troubleshooting"];
  for (const label of helpLabels) await page.getByRole("heading", { name: label }).waitFor();
  pass("Help tab exact sections", helpLabels.join(", "));
  await screenshot(page, "04-help-tab");
  ...
  await screenshot(page, "05-page-89-selected");
  ...
  await screenshot(page, "06-photo-uploaded");
  ...
  await page.mouse.up();
  for (const name of ["Resize +", "Resize -", "Rotate +", "Rotate -", "Reset Photo"]) await page.getByRole("button", { name }).click();
  pass("Drag, resize, rotate, reset", "Mouse drag plus Resize +/- Rotate +/- and Reset Photo controls completed.");
  await screenshot(page, "07-moved-resized-rotated");
  ...
  await screenshot(page, "08-crop-applied-canceled");

  const previewSelect = page.locator("select.restoration-input").nth(1);
  await previewSelect.selectOption("side-by-side");
  await screenshot(page, "09-preview-side-by-side");
  await previewSelect.selectOption("overlay");
  await screenshot(page, "10-preview-overlay");
  ...
  await screenshot(page, "11-draft-reopened");
  ...
  await screenshot(page, "12-approved-locked");
  ...
  await screenshot(page, "13-export-complete");
  ...
  await screenshot(page, "14-returned-draft-photo-removed");
  ...
  await screenshot(page, "15-admin-locked");
  await browser.close();
}
...
async function runMobile(inputs) {
  ...
  await screenshot(page, "16-mobile-iphone-tool");
  ...
  if (metrics.canvas?.width > 0 && !smallTouchTarget) pass("Mobile iPhone emulation", "Controls wrap, touch buttons are usable, canvas area is visible, and crop tap/cancel works.");
  else fail("Mobile iPhone emulation", JSON.stringify({ metrics, smallTouchTarget }).slice(0, 1600));
  await browser.close();
}
...
async function runTablet(inputs) {
  ...
  await page.locator('input[type="file"]').first().setInputFiles(inputs.png);
  await waitForUploadedFileName(page, "recovered-landscape.png");
  await screenshot(page, "17-tablet-tool");
...
async function runWebKit() {
  try {
    ...
    await screenshot(page, "18-webkit-pin");
    await browser.close();
...
async function writeReport(originalScanHashBefore, originalScanHashAfter, serverOutput) {
  ...
  const exportArtifact = "qa/v22.1-photo-tool-repair/example-page-002-approved-restoration-export.zip";
  ...
  const report = [
    ...
    "## Screenshots",
    "",
    ...screenshots.map((name) => `- qa/v22.1-photo-tool-repair/screenshots/${name}`),
    "",
    "## Export Artifact",
    "",
...
async function main() {
  ...
  console.log("Acceptance report written to qa/v22.1-photo-tool-repair/acceptance-report.md");
}
